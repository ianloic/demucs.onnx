...
static void reflect_padding(Eigen::MatrixXf& padded_mix, int left_padding,
...
Eigen::Tensor3dXf demucsonnx::demucs_inference(
    struct demucsonnx::demucs_model& model, const Eigen::MatrixXf& audio,
    demucsonnx::ProgressCallback cb) {
  int length = audio.cols();
  int max_shift = static_cast<int>(demucsonnx::MAX_SHIFT_SECS *
  ...
  padded_mix.block(0, max_shift, 2, length) =
      (audio.array() - ref_mean) / ref_std;
  ...
  Eigen::Ref<const Eigen::MatrixXf> shifted_audio =
      padded_mix.block(0, shift_offset, 2, shifted_length);

  // --- Begin merged split_inference and segment_inference logic ---

  // Calculate segment size in samples
  int segment_samples = static_cast<int>(demucsonnx::SEGMENT_LEN_SECS *
                                         demucsonnx::SUPPORTED_SAMPLE_RATE);
  int nb_out_sources = model.nb_sources;

  // Create reusable buffers with padded sizes
  demucsonnx::demucs_segment_buffers buffers(2, segment_samples,
                                             nb_out_sources);
  demucsonnx::stft_buffers stft_buf(buffers.padded_segment_samples);

  // Calculate stride for overlapping segments
  int stride_samples =
      static_cast<int>((1.0f - demucsonnx::OVERLAP) * segment_samples);
  ...
  weight.head(half_segment) =
      Eigen::VectorXf::LinSpaced(half_segment, 1, half_segment);
  ...
  int total_chunks = static_cast<int>(
      std::ceil(static_cast<float>(shifted_length) / stride_samples));
  ...
  for (int segment_offset = 0; segment_offset < shifted_length;
       segment_offset += stride_samples) {
    int chunk_length =
        std::min(segment_samples, shifted_length - segment_offset);

    // Create a block view for the current chunk
    Eigen::Ref<const Eigen::MatrixXf> chunk =
        shifted_audio.block(0, segment_offset, 2, chunk_length);
    ...
    buffers.padded_mix.block(0, buffers.pad + symmetric_padding_start, 2,
                             chunk_length) = chunk;

    // then, reflect padding on the left and right for
    // the stft boundary effects
    reflect_padding(buffers.padded_mix, buffers.pad, buffers.pad_end,
                    segment_samples);
    ...
    // Update progress
    if (!cb(inference_progress + increment_per_chunk,
            "Segment inference complete")) {
      std::cout << "Inference cancelled." << std::endl;
      break;
    }
    ...
    for (int i = 0; i < nb_out_sources; ++i) {
      for (int j = 0; j < 2; ++j) {
        ...
        Eigen::Map<Eigen::ArrayXf> out_slice(&out(i, j, segment_offset),
                                             chunk_length);
        // Map the (i, j, chunk_length) slice of 'chunk_out' to an
        // Eigen::ArrayXf
        Eigen::Map<const Eigen::ArrayXf> chunk_out_slice(&chunk_out(i, j, 0),
                                                         chunk_length);
    ...
    sum_weight.segment(segment_offset, chunk_length) +=
        weight.head(chunk_length);
