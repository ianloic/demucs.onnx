...
bool demucsonnx::load_model(const char* model_data, int n_bytes,
  ...
  model.sess = std::make_unique<Ort::Session>(model.env, final_data, final_size,
                                              session_options);

  std::vector<Ort::AllocatedStringPtr> input_name_allocs;
  input_name_allocs.push_back(
      model.sess->GetInputNameAllocated(0, demucsonnx::allocator));
  input_name_allocs.push_back(
      model.sess->GetInputNameAllocated(1, demucsonnx::allocator));

  model.input_names.push_back(
      input_name_allocs[0].get());  // Store as std::string
  model.input_names.push_back(input_name_allocs[1].get());

  std::vector<Ort::AllocatedStringPtr> output_name_allocs;
  output_name_allocs.push_back(
      model.sess->GetOutputNameAllocated(0, demucsonnx::allocator));
  output_name_allocs.push_back(
      model.sess->GetOutputNameAllocated(1, demucsonnx::allocator));
  ...
  auto output0_shape =
      model.sess->GetOutputTypeInfo(0).GetTensorTypeAndShapeInfo().GetShape();
  auto output1_shape =
      model.sess->GetOutputTypeInfo(1).GetTensorTypeAndShapeInfo().GetShape();
...
bool demucsonnx::load_model(const std::vector<char>& model_data,
                            struct demucsonnx::demucs_model& model,
                            Ort::SessionOptions& session_options) {
  return load_model(model_data.data(), model_data.size(), model,
                    session_options);
}

void RunONNXInference(struct demucsonnx::demucs_model& model,
                      struct demucsonnx::demucs_segment_buffers& buffers) {
  // Run the model
  model.sess->Run(demucsonnx::run_options, model.input_names_ptrs.data(),
                  buffers.input_tensors.data(), buffers.input_tensors.size(),
                  model.output_names_ptrs.data(), buffers.output_tensors.data(),
...
void demucsonnx::model_inference(
  ...
  // we're also dropping 2 bins from start and end i.e. the 2:2+le removal in
  // python
  ...
  std::vector<Eigen::MatrixXf> xt_3d(
      nb_out_sources, Eigen::MatrixXf(2, buffers.segment_samples));
  ...
  for (int s = 0; s < nb_out_sources; ++s) {  // loop over 4 sources
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < buffers.segment_samples; ++j) {
        int index =
            s * 2 * buffers.segment_samples + i * buffers.segment_samples + j;
  ...
  for (int source = 0; source < nb_out_sources; ++source) {
    Eigen::Tensor3dXcf z_target =
        Eigen::Tensor3dXcf(2, buffers.z.dimension(1), buffers.z.dimension(2));
    ...
    for (int i = 0; i < z_loop_dim_0; ++i) {
      for (int j = 0; j < z_loop_dim_1; ++j) {
        for (int k = 0; k < z_loop_dim_2; ++k) {
          int real_index =
              source * 2 * z_loop_dim_0 * z_loop_dim_1 * z_loop_dim_2 +
              2 * i * z_loop_dim_1 * z_loop_dim_2 + j * z_loop_dim_2 + k;
          int imag_index =
              source * 2 * z_loop_dim_0 * z_loop_dim_1 * z_loop_dim_2 +
              (2 * i + 1) * z_loop_dim_1 * z_loop_dim_2 + j * z_loop_dim_2 + k;
          z_target(i, j, k + 2) = std::complex<float>(x_out_data[real_index],
        ...
        z_target(i, j, buffers.z.dimension(2) - 1) =
            std::complex<float>(0.0f, 0.0f);
        z_target(i, j, buffers.z.dimension(2) - 2) =
            std::complex<float>(0.0f, 0.0f);
      }

      // set the entire slice along the last bin to zero
      for (int k = 0; k < buffers.z.dimension(2); ++k) {
        z_target(i, buffers.z.dimension(1) - 1, k) =
            std::complex<float>(0.0f, 0.0f);
      }
    }

    Eigen::MatrixXf padded_waveform =
        Eigen::MatrixXf(2, buffers.padded_segment_samples);

    demucsonnx::istft(stft_buf, z_target, padded_waveform);

    // copy target waveform into all 4 dims of targets_out
    // summing with xt_3d in the process to merge the time and frequency
    // branches
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < buffers.segment_samples; ++k) {
        buffers.targets_out(source, j, k) =
            padded_waveform(j, buffers.pad + k) + xt_3d[source](j, k);
      }
    }
  }
}
