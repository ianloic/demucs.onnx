...
using ProgressCallback = std::function<bool(float, const std::string&)>;
...
struct demucs_segment_buffers {
  ...
  demucs_segment_buffers(int nb_channels, int segment_samples, int nb_sources)
      : segment_samples(segment_samples),
        ...
        x_onnx_out_shape(
            {1, nb_sources, 2 * nb_channels, nb_stft_bins - 1, nb_stft_frames}),
        xt_onnx_out_shape({1, nb_sources, nb_channels, segment_samples}) {
    // precompute the input tensors
    // inputs in form (xt, x)
    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        demucsonnx::allocator, xt_onnx_in_shape.data(),
        xt_onnx_in_shape.size()));

    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        demucsonnx::allocator, x_onnx_in_shape.data(), x_onnx_in_shape.size()));

    // precompute the output tensors
    // outputs in form (x_out, xt_out)
    output_tensors.push_back(Ort::Value::CreateTensor<float>(
        demucsonnx::allocator, x_onnx_out_shape.data(),
        x_onnx_out_shape.size()));

    output_tensors.push_back(Ort::Value::CreateTensor<float>(
        demucsonnx::allocator, xt_onnx_out_shape.data(),
